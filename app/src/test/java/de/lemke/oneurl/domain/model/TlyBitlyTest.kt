...
class TlyBitlyTest {
    ...
    @Test
    fun `name reflects the bit_ly provider and it is disabled`() {
        Tly.Bitly.name shouldBe "t.ly (bit.ly)"
        Tly.Bitly.enabled shouldBe false
    }

    @Test
    fun `request body targets the bit_ly provider`() {
        val req = Tly.Bitly.getCreateRequest(context, longURL, "", { }, { fail("unexpected error") })

        JSONObject(String(req.body!!, Charsets.UTF_8)).getString("provider") shouldBe "bit.ly"
    }

    @Test
    fun `succeeds when the response contains a short_url`() {
        var result: String? = null
        val req = Tly.Bitly.getCreateRequest(context, longURL, "", { result = it }, { fail("unexpected error: $it") })
