...
class SketchyTest {
    ...
    fun `create request succeeds when the response has an id`() {
        ...
        val req = Owovc.Sketchy.getCreateRequest(context, longURL, "", { result = it }, { fail("unexpected error: $it") })

        req.deliverJSONResponse(JSONObject().put("id", " uwu.owo.vc/sketchy-uvu "))

        result shouldBe "uwu.owo.vc/sketchy-uvu"
    }

    @Test
    fun `create request fails with Unknown when the response has no id`() {
        var error: GenerateURLError? = null
        val req = Owovc.Sketchy.getCreateRequest(context, longURL, "", { fail("unexpected success") }, { error = it })
        ...
        val cases =
            listOf<Pair<VolleyError, GenerateURLError>>(
                ...
                VolleyError(NetworkResponse(503, ByteArray(0), false, 0L, emptyList())) to
                    GenerateURLError.ServiceTemporarilyUnavailable(Owovc.Sketchy.baseURL),
    ...
    @Test
    fun `getTipsCardTitleAndInfo returns sketchy specific info`() {
        val (title, info) = requireNotNull(Owovc.Sketchy.getTipsCardTitleAndInfo(context))

        title shouldBe context.getString(commonutilsR.string.commonutils_info)
        info shouldBe context.getString(R.string.owovc_sketchy)
    }

    @Test
    fun `getInfoContents returns the sketchy and analytics info`() {
        val infoContents = Owovc.Sketchy.getInfoContents(context)

        infoContents.size shouldBe 2
        infoContents[0].title shouldBe "sketchy"
        infoContents[0].linkOrDescription shouldBe context.getString(R.string.owovc_sketchy)
