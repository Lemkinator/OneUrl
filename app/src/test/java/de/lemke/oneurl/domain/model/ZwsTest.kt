...
class ZwsTest {
    ...
    fun `create request succeeds when the response has an id`() {
        ...
        val req = Owovc.Zws.getCreateRequest(context, longURL, "", { result = it }, { fail("unexpected error: $it") })

        req.deliverJSONResponse(JSONObject().put("id", " uwu.owo.vc/uwU/uwU_Ovo/uvu "))

        result shouldBe "uwu.owo.vc/uwU/uwU_Ovo/uvu"
    }

    @Test
    fun `create request fails with Unknown when the response has no id`() {
        var error: GenerateURLError? = null
        val req = Owovc.Zws.getCreateRequest(context, longURL, "", { fail("unexpected success") }, { error = it })
        ...
        val cases =
            listOf<Pair<VolleyError, GenerateURLError>>(
                ...
                VolleyError(NetworkResponse(503, ByteArray(0), false, 0L, emptyList())) to
                    GenerateURLError.ServiceTemporarilyUnavailable(Owovc.Zws.baseURL),
    ...
    @Test
    fun `getTipsCardTitleAndInfo returns zws specific info`() {
        val (title, info) = requireNotNull(Owovc.Zws.getTipsCardTitleAndInfo(context))

        title shouldBe context.getString(commonutilsR.string.commonutils_info)
        info shouldBe context.getString(R.string.owovc_zws)
    }

    @Test
    fun `getInfoContents returns the zws and analytics info`() {
        val infoContents = Owovc.Zws.getInfoContents(context)

        infoContents.size shouldBe 2
        infoContents[0].title shouldBe "zws"
        infoContents[0].linkOrDescription shouldBe context.getString(R.string.owovc_zws)
