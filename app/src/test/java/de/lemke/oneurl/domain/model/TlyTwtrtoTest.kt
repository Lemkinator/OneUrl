...
class TlyTwtrtoTest {
    ...
    @Test
    fun `name and enabled reflect the twtr_to provider`() {
        Tly.Twtrto.name shouldBe "t.ly (twtr.to)"
        Tly.Twtrto.enabled shouldBe true
    }

    @Test
    fun `request body targets the twtr_to provider`() {
        val req = Tly.Twtrto.getCreateRequest(context, longURL, "", { }, { fail("unexpected error") })

        JSONObject(String(req.body!!, Charsets.UTF_8)).getString("provider") shouldBe "twtr.to"
    }

    @Test
    fun `succeeds when the response contains a short_url`() {
        var result: String? = null
        val req = Tly.Twtrto.getCreateRequest(context, longURL, "", { result = it }, { fail("unexpected error: $it") })
