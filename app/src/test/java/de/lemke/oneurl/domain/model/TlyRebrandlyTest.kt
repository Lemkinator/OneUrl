...
class TlyRebrandlyTest {
    ...
    @Test
    fun `name reflects the rebrand_ly provider and it is disabled`() {
        Tly.Rebrandly.name shouldBe "t.ly (rebrand.ly)"
        Tly.Rebrandly.enabled shouldBe false
    }

    @Test
    fun `request body targets the rebrand_ly provider`() {
        val req = Tly.Rebrandly.getCreateRequest(context, longURL, "", { }, { fail("unexpected error") })

        JSONObject(String(req.body!!, Charsets.UTF_8)).getString("provider") shouldBe "rebrand.ly"
    }

    @Test
    fun `succeeds when the response contains a short_url`() {
        var result: String? = null
        val req = Tly.Rebrandly.getCreateRequest(context, longURL, "", { result = it }, { fail("unexpected error: $it") })
