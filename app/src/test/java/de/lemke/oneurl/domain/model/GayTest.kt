...
class GayTest {
    ...
    fun `create request succeeds when the response has an id`() {
        ...
        val req = Owovc.Gay.getCreateRequest(context, longURL, "", { result = it }, { fail("unexpected error: $it") })

        req.deliverJSONResponse(JSONObject().put("id", " uwu.owo.vc/gay-uvu "))

        result shouldBe "uwu.owo.vc/gay-uvu"
    }

    @Test
    fun `create request fails with Unknown when the response has no id`() {
        var error: GenerateURLError? = null
        val req = Owovc.Gay.getCreateRequest(context, longURL, "", { fail("unexpected success") }, { error = it })
        ...
        val cases =
            listOf<Pair<VolleyError, GenerateURLError>>(
                ...
                VolleyError(NetworkResponse(503, ByteArray(0), false, 0L, emptyList())) to
                    GenerateURLError.ServiceTemporarilyUnavailable(Owovc.Gay.baseURL),
                VolleyError(NetworkResponse(500, ByteArray(0), false, 0L, emptyList())) to GenerateURLError.Unknown(500),
                VolleyError(
                    NetworkResponse(400, """{"message":"body/link must match pattern"}""".toByteArray(), false, 0L, emptyList()),
    ...
    @Test
    fun `getTipsCardTitleAndInfo returns gay specific info`() {
        val (title, info) = requireNotNull(Owovc.Gay.getTipsCardTitleAndInfo(context))

        title shouldBe context.getString(commonutilsR.string.commonutils_warning)
        info shouldBe context.getString(R.string.owovc_gay)
    }

    @Test
    fun `getInfoContents returns the gay and analytics info`() {
        val infoContents = Owovc.Gay.getInfoContents(context)

        infoContents.size shouldBe 2
        infoContents[0].title shouldBe "gay"
        infoContents[0].linkOrDescription shouldBe context.getString(R.string.owovc_gay)
        infoContents[1].title shouldBe context.getString(R.string.analytics)
        infoContents[1].linkOrDescription shouldBe context.getString(R.string.analytics_text)
    }
}
