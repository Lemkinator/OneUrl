...
// own. Both scope and defaultDispatcher are UnconfinedTestDispatcher, so bindQrCode's coroutine
// runs to completion synchronously - no idling required.
@HiltAndroidTest
...
class URLAdapterTest {
    ...
    private fun withAdapter(block: (URLAdapter, URLAdapter.ViewHolder) -> Unit) {
    ...
    fun `onViewRecycled cancels a pending qr load`() {
        ...
        withAdapter { adapter, holder ->
            adapter.submitList(listOf(testUrl("https://short.url/recycle")))
